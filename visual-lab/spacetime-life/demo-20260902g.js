...
(function initializeSpacetimeVisualizer() {
  ...
  const three = {
    ...
    oldColor: new THREE.Color("#748096"),
    middleColor: new THREE.Color("#acb2c0"),
    recentColor: new THREE.Color("#e5e9f1"),
    currentColor: new THREE.Color("#ffffff"),
    birthColor: new THREE.Color("#f0f4fb")
  };
  ...
  function colorForInstance(layerIndex, finalLayerIndex, wasBorn) {
    ...
    if (wasBorn) three.color.lerp(three.birthColor, 0.18);
    return three.color;
  }
  ...
  function initializeThree() {
    try {
      ...
      three.renderer.setClearColor(0x03050a, 1);
      ...
      // InstancedMesh enables instanceColor automatically after setColorAt().
      // BoxGeometry has no vertex color attribute, so vertexColors must remain disabled.
